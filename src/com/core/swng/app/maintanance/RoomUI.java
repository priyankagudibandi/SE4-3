...
public class RoomUI extends JFrame {
	...
	public void refreshTable() {
		...
		Room room = null;
		for (int i = 0; i < DataManager.roomsDataBaseMap.size(); i++) {
			 room = DataManager.roomsDataBaseMap.get(i);
	...
	public void create() {
		...
		if (isValidData()) {
			int i = DataManager.roomsDataBaseMap.size();
			DataManager.roomsDataBaseMap.put(i, room);
			refreshTable();
	...
	public void read() {
		if (getSelectedId() >= 0) {
			Room room = DataManager.roomsDataBaseMap.get(getSelectedId());
			cleanFields();
	...
	public void update() {
		if (getSelectedId() >= 0) {
			...
			room.setCapacity(textCapacity.getText());
			room.setRoomNumber(textRoomNumber.getText());
			DataManager.roomsDataBaseMap.put(getSelectedId(), room);
	...
	public void delete() {
		...
		int selectedRow = jTable.getSelectedRow();
		if (selectedRow >= 0) {
			DataManager.roomsDataBaseMap.remove(getSelectedId());
